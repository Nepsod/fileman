            ...
            if cached_icon.is_none() {
                let spawn_icon_load = {
                    let mut pending = self
                        .pending_icon_loads
                        .lock()
                        .expect("Failed to lock pending_icon_loads in view_compact");
                    ...
                    tokio::spawn(async move {
                        ...
                        if let Ok(file) = get_file_for_uri(&uri) {
                            ...
                            let mut cache = cache_clone.lock().expect(
                                "Failed to lock icon_cache in async task (view_compact)",
                            );
                        ...
                        pending_icon_loads
                            .lock()
                            .expect("pending_icon_loads unlock (view_compact)")
