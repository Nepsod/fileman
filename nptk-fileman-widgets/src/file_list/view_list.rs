            ...
            if cached_icon.is_none() {
                let spawn_icon_load = {
                    let mut pending = self
                        .pending_icon_loads
                        .lock()
                        .expect("Failed to lock pending_icon_loads in view_list");
                ...
                if spawn_icon_load {
                    ...
                    tokio::spawn(async move {
                        ...
                        if let Ok(file) = get_file_for_uri(&uri) {
                            let icon = registry_clone
                                .get_file_icon(&*file, icon_size as u32)
                                .await;
                            let mut cache = cache_clone.lock().expect(
                                "Failed to lock icon_cache in async task (view_list)",
                            );
                        ...
                        pending_icon_loads
                            .lock()
                            .expect("pending_icon_loads unlock (view_list)")
